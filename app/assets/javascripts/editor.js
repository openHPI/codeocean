$(function() {
  ...
  var evaluateCodeWithStreamedResponse = function(url, callback) {
    ...
    event_source.addEventListener('info', initializeWebsocket);
  ...
  var initializeWebsocket = function(event) {
    var data = JSON.parse(event.data);
    initWebsocketConnection(data["container"]);
    initTurtle();
  }

  var initWebsocketConnection = function(container_id) {
      // todo stream full websocket uri
      var wsUri = 'ws://' + window.location.hostname + ':3333/socket/' + container_id;
