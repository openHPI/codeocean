...
## file_name, file_role, file_ext, file_readonly, file_content
...
submissions_data.each{|row|
	(submissions_hash[row["exercises.id"]] ||= []) << [row["file_name"], row["file_ext"], row["file_readonly"], row["file_role"] ]
}
...
submissions_hash.each do |user_id, files|
	...
	#nsystem 'mkdir', '-p', "projects/#{user_id}/bin"
