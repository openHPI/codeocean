class CppCatch2Adapter < TestingFrameworkAdapter
  ...
  def parse_output(output)
    if COUNT_REGEXP.match(output[:stdout])
      {count: Regexp.last_match(1).to_i, passed: Regexp.last_match(1).to_i}
    else
      count = COUNT_REGEXP.match(output[:stdout]).try(:captures).try(:first).try(:to_i) || FAILURES_REGEXP.last_match(1).try(:captures).try(:first).try(:to_i) || BOTH_REGEXP.last_match(1).try(:captures).try(:first).try(:to_i) || 0
      failed = FAILURES_REGEXP.match(2).try(:captures).try(:first).try(:to_i) || BOTH_REGEXP.match(3).try(:captures).try(:first).try(:to_i) || 0
